package org.nasdanika.models.bank.generator.tests;
...
import org.nasdanika.models.bank.processors.ecore.EcoreGenBankProcessorsFactory;
 ...
 *
 */
public class TestBankModelDocGen {
	
	@Test
	public void testGenerateBankModelDoc() throws IOException, DiagnosticException {
		ProgressMonitor progressMonitor = new NullProgressMonitor(); // new PrintStreamProgressMonitor();
		MutableContext context = Context.EMPTY_CONTEXT.fork();
		Consumer<Diagnostic> diagnosticConsumer = d -> d.dump(System.out, 0);
		List<Function<URI,Action>> actionProviders = new ArrayList<>();		
		EcoreGenBankProcessorsFactory ecoreGenBankProcessorFactory = new EcoreGenBankProcessorsFactory(context);		
		EcoreNodeProcessorFactory ecoreNodeProcessorFactory = new EcoreNodeProcessorFactory(
				...
				diagnosticConsumer,
				ecoreGenBankProcessorFactory);
		
		File actionModelsDir = new File("target\\action-models\\");
		actionModelsDir.mkdirs();
		File output = new File(actionModelsDir, "bank.xmi");
		...
		String siteMapDomain = "https://bank.models.nasdanika.org";		
